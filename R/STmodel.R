...
##' @param scale Scale the covariates? If \code{TRUE} all numeric covariates
...
createSTmodel <- function(STdata, LUR=NULL, ST=NULL,
  ...
  if( scale ){
    if( is.null(scale.covars) ){
      ##mean and sd -> return NA if not numeric (i.e. text or factors)
      mean.covars <- sapply(STmodel$covars, function(x){ 
        if(is.numeric(x)){ mean(x) }else{ NA }
        })
      sd.covars <- sapply(STmodel$covars, function(x){ 
        if(is.numeric(x)){ sd(x) }else{ NA }
        })
      ##only scale covariates that are numeric and have mean+sd != NA
      no.scale.covars <- is.na(mean.covars) | is.na(sd.covars)
      mean.covars[no.scale.covars] <- NA
      sd.covars[no.scale.covars] <- NA
      ##save for future use
